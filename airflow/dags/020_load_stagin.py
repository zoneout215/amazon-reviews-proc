...
ITEMS_RAW_TABLE = 'e-analogy-449921-p7.amazon_reviews.raw_data_metadata'
...
with DAG(**dag_config) as dag:
    ...
    load_json_to_bigquery = GCSToBigQueryOperator(
            ...
            autodetect=True,
            source_format='NEWLINE_DELIMITED_JSON',  # For JSONL files
    ...
    load_metadata = BigQueryExecuteQueryOperator(
        ...
        sql="""
            TRUNCATE TABLE `e-analogy-449921-p7.amazon_reviews.raw_data_metadata`;
            LOAD DATA OVERWRITE `e-analogy-449921-p7.amazon_reviews.raw_data_metadata` (json_string STRING)
            FROM FILES (
            format = 'CSV',
            field_delimiter = '\\u00FE', 
            quote = '',
